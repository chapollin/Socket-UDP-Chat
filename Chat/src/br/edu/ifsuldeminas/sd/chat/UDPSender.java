	...
	public void send(String message) throws ChatException {
		...
		try {
			...
			throw new ChatException(String.format(
					"Sua mensagem ’%s’ nao foi enviada.", message), ioException);
		}
	}
	private void validateAttributes(InetAddress receiver, int receiverPort) {
		if (receiver == null)
			throw new IllegalArgumentException("O receiver nao pode ser nulo.");
		if (receiverPort <= 0)
			throw new IllegalArgumentException(
					String.format("Porta invalida para o receiver."));
	}
}